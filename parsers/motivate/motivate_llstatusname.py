...
def get(bsr_df, output='file', save_dir='', timeout_sec=20, apikey='', save_raw=False, save_raw_dir=''):
    ...
    for stn_dict in stations_list:
        ...
        clean_stations_list.append([str(int(stnid)), str(stn_dict['latitude']), str(stn_dict['longitude']), str(stn_dict['availableBikes']), str(stn_dict['availableDocks']), stn_dict['stationName']])
    ...
    output = u'id\tlat\tlong\tbikes\tspaces\tname\n'
